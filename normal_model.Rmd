...
MCMC variance
$$\begin{aligned}
\mbox{Var}_{\mbox{MCMC}}(\bar{\phi}) &= \mbox{Var}_{\mbox{MC}}(\bar{\phi}) + \frac{1}{S^2}\mbox{E}\{(\phi^{(s)}-\phi_0)(\phi^{(t)}-\phi_0)\} = \frac{\mbox{Var}({\phi})}{S_{\mbox{eff}}}\\
...
$$\mbox{acf}_t(\phi) = \frac{\frac{1}{S-t}\sum_{s=1}^{S-t}(\phi^{(s)}-\bar{\phi})(\phi^{(t)}-\bar{\phi})}{\frac{1}{S-1}\sum_{s=1}^{S}(\phi^{(s)}-\bar{\phi})^2}$$
```{r atuocorrelation}
par(mfrow = c(1,2), mar = c(3,3,1,1), mgp = c(1.75,0.75,0))
...
$$p(\theta,\delta) = (\mathrm{dnorm}(\theta,\mu_1,\sigma_1))^{I(d=1)} (\mathrm{dnorm}(\theta,\mu_2,\sigma_2))^{I(d=2)} (\mathrm{dnorm}(\theta,\mu_3,\sigma_3))^{I(d=3)}$$
